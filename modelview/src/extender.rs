//! Framework's battery extender: what its reading is called.

use frameguin_contract::{ExtenderStage, ExtenderState};
use frameguin_model::control::battery::extender::stage_window;
...
pub fn stage_label(state: ExtenderState, charge_limit: Option<u8>) -> String {
    ...
    let (name, (lower, upper)) = match (state.stage, stage_window(state.stage)) {
        (ExtenderStage::First, Some(window)) => ("Stage 1", window),
        (ExtenderStage::Second, Some(window)) => ("Stage 2", window),
        (ExtenderStage::Inactive, _) | (_, None) => return "Waiting".to_owned(),
    };
    match charge_limit {
        Some(limit) if limit <= upper => {
            format!(
